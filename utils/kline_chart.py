...
def generate_kline_chart(
    ...
    for i, row in df.iterrows():
        ...
        # 关键K线标记（星号）
        if row["date"] in key_candle_dates or (
            isinstance(row["date"], pd.Timestamp)
            and row["date"].strftime("%Y-%m-%d")
            in [
                d.strftime("%Y-%m-%d") if isinstance(d, pd.Timestamp) else d
                for d in key_candle_dates
            ]
        ):
            ax_kline.scatter(
                i,
