...
from research.legacy.pattern_config import B1_PERFECT_CASES, SIMILARITY_WEIGHTS
from research.legacy.pattern_feature_extractor import PatternFeatureExtractor
from research.legacy.pattern_matcher import PatternMatcher
...
class B1PatternLibrary:
    ...
    def __init__(self, csv_manager):
        ...
        self.cache_file = (
            Path(csv_manager.base_data_dir) / "b1_pattern_library_cache.json"
        )
        ...
        for case in B1_PERFECT_CASES:
            try:
                ...
                if window_df.empty or len(window_df) < 10:
                    print(
                        f"  ⚠️ 跳过 {case['name']}: 日期 {case['breakout_date']} 附近数据不足"
                    )
                    continue
    ...
    def find_best_match(
        self, stock_code: str, stock_df: pd.DataFrame, lookback_days: int = 25
    ) -> dict:
        """
        为单只股票找到最匹配的B1完美图形案例
        ...
        candidate_features = self.extractor.extract(
            stock_df, lookback_days=lookback_days
        )
        ...
        for case_id, case_data in self.cases.items():
            try:
                similarity = self.matcher.match(
                    candidate_features, case_data["features"]
                )
    ...
    def add_case(self, case_config: dict):
        ...
        try:
            ...
            window_df = self._extract_window(
                df, case_config["breakout_date"], case_config.get("lookback_days", 25)
            )
            features = self.extractor.extract(window_df)
