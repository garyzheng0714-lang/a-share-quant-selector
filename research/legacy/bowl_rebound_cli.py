...
def _legacy_root() -> Path:
    from research.legacy.isolation import legacy_root
...
class QuantSystem:
    ...
    def __init__(self, research_root: Path, config_file=None):
        from research.legacy.bowl_rebound_strategy import BowlReboundStrategy
        from research.legacy.strategy_registry import get_registry
        from utils.akshare_fetcher import AKShareFetcher
        from utils.csv_manager import CSVManager

        self.research_root = research_root
        self.config = self._load_config(config_file)
        self.data_dir = str(research_root / "data")
        self.csv_manager = CSVManager(self.data_dir)
        self.fetcher = AKShareFetcher(self.data_dir)
        self.notifier = self._init_notifier()
        # 旧策略参数也必须来自仓库外研究目录，不能读取生产配置。
        self.registry = get_registry(str(research_root / "strategy_params.yaml"))
        self.registry.register(BowlReboundStrategy)
    ...
    def _load_config(self, config_file):
        ...
        if self.research_root not in config_path.parents:
            raise RuntimeError("legacy_config_must_be_inside_research_root")
    ...
    def _init_notifier(self):
        ...
        if os.environ.get("ALLOW_LEGACY_NOTIFICATIONS") != "1":
    ...
    def _smart_update(self, max_stocks=None, check_latest=True):
        ...
        if check_latest:
            ...
            if check_limit > 0 and has_today == check_limit:
                ...
                print(
                    f"  已检查 {check_limit} 只，{has_today} 只有今天数据，{no_today} 只需要更新"
                )
    ...
    def select_stocks(self, category="all", max_stocks=None, return_data=False):
        ...
        if mb_only:
            ...
            print(
                f"板块过滤：只保留沪深主板 {len(stock_codes)} 只（筛掉 {before - len(stock_codes)} 只创业板/科创板/北交所）"
            )
        ...
        for strategy_name, strategy in self.registry.strategies.items():
            ...
            for i, code in enumerate(process_codes, 1):
                ...
                if signal_list:
                    for s in signal_list:
                        ...
                        if category == "all" or cat == category:
                            signals.append({"code": code, "name": name, "signals": [s]})
                            # 只保存入选股票的数据
                            if return_data:
                ...
                if i % 100 == 0 or i == len(process_codes):
                    gc.collect()
                    print(
                        f"  进度: [{i}/{len(process_codes)}] 有效 {valid_count} 只，选出 {len(signals)} 只..."
                    )
        ...
        for strategy_name, signals in results.items():
            ...
            for signal in signals:
                ...
                for s in signal["signals"]:
                    ...
                    print(
                        f"  {cat_emoji} {code} {name}: 价格={s['close']}, J={s['J']}, 理由={s['reasons']}"
                    )
    ...
    def _persist_and_track(self, results, stock_names, matched=None):
        ...
        try:
            from research.legacy import view_manager
            from research.legacy.performance_tracker import update_performance
                        ...
                        stocks.append(
                            {
                                "code": sig["code"],
                                "name": sig.get(
                                    "name", stock_names.get(sig["code"], "未知")
                                ),
            ...
            print(
                f"📊 战绩回填：新增 {stats['synced']} 条追踪，更新 {stats['updated']} 条"
            )
    ...
    def select_with_b1_match(
        self, category="all", max_stocks=None, min_similarity=None, lookback_days=None
    ):
        """
        执行选股 + B1完美图形匹配排序
        ...
        """
        # 从配置读取默认值
        from research.legacy.pattern_config import (
        ...
        print("\n[2/3] 初始化B1完美图形库...")
        try:
            from research.legacy.pattern_config import MIN_SIMILARITY_SCORE
            from research.legacy.pattern_library import B1PatternLibrary
            ...
            for signal in signals:
                ...
                try:
                    ...
                    match_result = library.find_best_match(
                        code, df, lookback_days=lookback_days
                    )
        ...
        # 显示Top N结果（使用配置）
        from research.legacy.pattern_config import TOP_N_RESULTS
        ...
        if matched_results:
            ...
            for i, r in enumerate(matched_results[:TOP_N_RESULTS], 1):
                ...
                print(
                    f"   相似度: {r['similarity_score']}% | 匹配: {r['matched_case']}"
                )
                bd = r.get("breakdown", {})
                print(
    ...
    def run_with_b1_match(
        self, category="all", max_stocks=None, min_similarity=60.0, lookback_days=25
    ):
        """
        ...
        if match_result.get("matched"):
            ...
            self.notifier.send_b1_match_results(
                match_result["matched"], match_result.get("total_selected", 0)
            )
            print("✓ 通知发送完成")
        else:
            print("\n⚠️ 没有匹配结果，跳过通知")
...
B1完美图形匹配:
  ...
  使用 --min-similarity 调整匹配阈值（默认60%，范围0-100）
    ...
    parser.add_argument(
        "command",
        choices=["init", "update", "run", "track", "backtest"],
        nargs="?",
    ...
    parser.add_argument("--config", default=None, help="隔离研究目录内的非生产配置文件")

    parser.add_argument(
        "--host", default="0.0.0.0", help="Web服务器监听地址 (默认: 0.0.0.0)"
    )
    ...
    research_root = _legacy_root()
    from research.legacy import view_manager

    view_manager.configure_database(research_root)

    # 切换工作目录
    os.chdir(project_root)

    # 创建系统实例
    quant = QuantSystem(research_root, args.config)
    ...
    if args.command == "init":
        ...
        # 回填战绩并打印摘要
        from research.legacy.performance_tracker import (
            get_summary,
            print_summary,
            update_performance,
        )
        ...
        print(
            f"✓ 回填完成: 新增 {stats['synced']} 条追踪, 更新 {stats['updated']} 条\n"
        )
        ...
        from research.legacy.backtest_engine import format_report, run_backtest

        start = args.start or (datetime.now() - timedelta(days=730)).strftime(
            "%Y-%m-%d"
        )
        end = args.end or datetime.now().strftime("%Y-%m-%d")

        # 股票名字只用本地缓存，不碰网络（避免卡死）
        names_file = Path(quant.data_dir) / "stock_names.json"
        stock_names = (
        ...
        if sample_codes:
            caps = [
                quant.csv_manager.read_stock(c, nrows=5)
                .get("market_cap", pd.Series([0]))
                .max()
        ...
        strategy_params = {}
        params_file = quant.research_root / "strategy_params.yaml"
        if params_file.exists():
            import yaml

            strategy_params = (
                yaml.safe_load(params_file.read_text(encoding="utf-8")) or {}
            ).get("BowlReboundStrategy", {})
        ...
        out_dir = Path(quant.data_dir) / "backtest"
