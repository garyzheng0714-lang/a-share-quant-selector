...
from research.legacy.performance_tracker import WINDOWS, _aggregate, _evaluate_one
...
def run_backtest(
    ...
    Args:
        ...
        ignore_cap: 已废弃；缺少 PIT 市值时必须 fail closed
    ...
    if ignore_cap:
        raise ValueError("missing_point_in_time_market_cap")
    ...
    for i, code in enumerate(codes, 1):
        ...
        if df.empty:
            continue
        if (
            "market_cap" not in df
            or not pd.to_numeric(df["market_cap"], errors="coerce").gt(0).any()
        ):
            # 旧回测没有可信 PIT 市值时不产生任何发布证据。
            continue
...
def format_report(summary: dict) -> str:
    ...
    lines = [
        ...
        "- 买入口径: 信号次日开盘价买入；收益为 T+n 收盘相对买入价",
    ]
    if summary["ignore_cap"]:
        lines.append(
            "- ⚠️ 本次回测**未启用市值门槛条件**（本地数据缺市值字段），"
    ...
    def table(agg):
        ...
        for w in WINDOWS:
            ...
            if a["count"]:
                rows.append(
                    f"| T+{w} | {a['count']} | {a['win_rate']}% | {a['avg']:+.2f}% |"
                )
            else:
                rows.append(f"| T+{w} | 0 | - | - |")
        return rows
    ...
    lines += [
        "",
        "> 注: 同一股票可能连续多日出信号（与实盘一致），样本间存在相关性；",
