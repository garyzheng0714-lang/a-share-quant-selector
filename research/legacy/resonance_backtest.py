...
该脚本使用旧的收盘退出口径，不符合当前 `a-share-eod-open-open-v5`，
因此已从生产 tools 移出。
"""
...
def main():
    ...
    ap.add_argument(
        "--skip", type=int, default=21, help="跳过最近N日（留T+20回填空间）"
    )
    ...
    output = legacy_path(args.out, "outputs/resonance_backtest.json")
    cm = CSVManager(data_dir, resolve_snapshot=False, writable=False)
    with (data_dir / "stock_names.json").open(encoding="utf-8") as handle:
    ...
    print(
        f"信号日区间: {signal_dates[-1]} ~ {signal_dates[0]}（{len(signal_dates)}天）"
    )
    ...
    print(
        f"股票数: {len(tasks)}，预计 {len(tasks) * len(signal_dates) * 8 / 1000 / 60 / 8:.0f} 分钟"
    )
    ...
    with ThreadPoolExecutor(max_workers=8) as ex:
        for res in ex.map(_one_stock, tasks):
            ...
            if done % 500 == 0:
                print(
                    f"  [{done}/{len(tasks)}] 信号 {len(records)} 条 "
                    f"({time.time() - t0:.0f}s)",
                    flush=True,
                )
    ...
    result = {
        ...
        "by_n_factors": _agg(
            records, lambda r: bucket_n(r["n_factors"]), "按命中因子总数（含同源重复）"
        ),
        "by_n_families": _agg(
            records,
            lambda r: bucket_n(r["n_families"]),
            "按独立逻辑家族数（真共振口径）",
        ),
        "by_single_factor": _agg(
            [r for r in records if r["n_factors"] == 1],
            lambda r: r["factors"][0],
            "单因子命中时各因子的独立战绩",
        ),
    ...
    for section in ("baseline", "by_n_factors", "by_n_families"):
        ...
        print(f"\n=== {s['label']} ===")
        print(
            f"{'档位':<8}{'样本':>7}{'T+1胜率':>9}{'T+1均值':>9}"
            f"{'T+5胜率':>9}{'T+5均值':>9}{'T+20胜率':>10}{'T+20均值':>10}{'均回撤':>8}"
        )
        for row in s["rows"]:
            print(
                f"{row['bucket']:<8}{row['n_signals']:>7}"
