...
class PatternFeatureExtractor:
    ...
    def _extract_trend_features(self, df: pd.DataFrame) -> dict:
        ...
        short_bullbear_ratio = (
            latest["short_term_trend"] / latest["bull_bear_line"]
        ...
        short_slope = (
            (df["short_term_trend"].iloc[-1] / df["short_term_trend"].iloc[-5] - 1)
            * 100
            if df["short_term_trend"].iloc[-5] != 0
            else 0
        )
        bullbear_slope = (
            (df["bull_bear_line"].iloc[-1] / df["bull_bear_line"].iloc[-5] - 1) * 100
            if df["bull_bear_line"].iloc[-5] != 0
            else 0
        )
        ...
        # 价格高于趋势线为正，低于为负
        price_vs_short_pct = (
            (latest["close"] - latest["short_term_trend"])
            / latest["short_term_trend"]
            * 100
            if latest["short_term_trend"] != 0
            else 0
        )
        price_vs_bullbear_pct = (
            (latest["close"] - latest["bull_bear_line"])
            / latest["bull_bear_line"]
            * 100
            if latest["bull_bear_line"] != 0
            else 0
        )
        ...
        # 4. 是否在碗中（短期趋势 > 价格 > 多空线）
        is_in_bowl = (
            latest["short_term_trend"] > latest["close"] > latest["bull_bear_line"]
        )
        ...
        price_bias_pct = (
            (latest["close"] - avg_trend) / avg_trend * 100 if avg_trend != 0 else 0
        )
    ...
    def _extract_kdj_features(self, df: pd.DataFrame) -> dict:
        ...
        if len(j_values) >= 5:
            ...
            recent_j = j_values[-5:]
            j_trend = (
                np.polyfit(x, recent_j, 1)[0] if not np.isnan(recent_j).any() else 0
            )
        else:
            j_trend = 0
    ...
    def _extract_volume_features(self, df: pd.DataFrame) -> dict:
        ...
        for i in range(len(df)):
            if (
                i > 0
                and df["volume"].iloc[i] > df["volume"].iloc[i - 1] * 2
    ...
    def _extract_shape_features(self, df: pd.DataFrame) -> dict:
        ...
        # 突破力度（最后一日涨幅）
        breakout_strength = (
            (closes[-1] / closes[-2] - 1) * 100 if len(closes) >= 2 else 0
        )
    ...
    def _count_consolidation_days(self, df: pd.DataFrame) -> int:
        ...
        for i in range(len(df) - 5):
            ...
            if (
                window_max > 0
                and (window_max - window_min) / window_max < consolidation_range
            ):
                current_days += 1
