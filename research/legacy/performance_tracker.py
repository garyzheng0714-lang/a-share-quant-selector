...
from research.legacy.view_manager import _get_conn
...
def init_performance_table() -> None:
    ...
    with _get_conn() as conn:
        ...
        existing = {
            r["name"] for r in conn.execute("PRAGMA table_info(performance)").fetchall()
        }
        columns = {
...
def _sync_from_results(csv_manager) -> int:
    ...
    with _get_conn() as conn:
        ...
        data_ver = data_version(csv_manager.data_dir)
        for row in rows:
            try:
...
def update_performance(csv_manager) -> dict:
    ...
    for code, items in by_code.items():
        ...
        if df.empty:
            continue
        keep = [
            c for c in ("date", "open", "high", "low", "close", "volume") if c in df
        ]
        daily = df[keep].copy()
    ...
    logger.info(
        "战绩回填完成: %s（invalidated=买入价<=0无法计算、已标记终态不再重复处理）",
        stats,
    )
    return stats
...
def get_summary(view_id=None) -> dict:
    ...
    for r in rows:
        ...
        by_bucket.setdefault(_bucket_similarity(r.get("similarity_score")), []).append(
            r
        )
...
def print_summary(summary: dict) -> None:
    ...
    def _print_block(title, agg):
        ...
        for n in WINDOWS:
            ...
            if a["count"]:
                parts.append(
                    f"T+{n}: 胜率{a['win_rate']}% 均{a['avg']:+.2f}% ({a['count']}条)"
                )
        print(f"  {title}")
