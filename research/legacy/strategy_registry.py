...
class StrategyRegistry:
    ...
    def __init__(self, params_file=None):
        ...
        self.params_file = (
            Path(params_file)
            if params_file
            else Path(__file__).with_name("strategy_params.example.yaml")
        )
    ...
    def auto_register_from_directory(self, strategy_dir="strategy"):
        ...
        if os.environ.get("ALLOW_LEGACY_RESEARCH") != "1":
            raise RuntimeError("legacy_strategy_registry_disabled")
    ...
    def run_strategy(self, strategy_name, stock_data_dict):
        ...
        for code, (name, df) in stock_data_dict.items():
            ...
            if processed % 100 == 0 or processed == total_stocks:
                print(
                    f"  进度: [{processed}/{total_stocks}] 已分析 {processed} 只，选出 {len(signals)} 只..."
                )
...
def get_registry(params_file=None):
    """获取全局策略注册器"""
    global _registry
