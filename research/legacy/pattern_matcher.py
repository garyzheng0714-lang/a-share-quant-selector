...
class PatternMatcher:
    ...
    def match(self, candidate_features: dict, case_features: dict) -> dict:
        ...
        # 1. 知行趋势线结构相似度
        if candidate_features.get("trend_structure") and case_features.get(
            "trend_structure"
        ):
            scores["trend_structure"] = self._calc_trend_similarity(
                candidate_features["trend_structure"], case_features["trend_structure"]
            )
        else:
        ...
        if candidate_features.get("kdj_state") and case_features.get("kdj_state"):
            scores["kdj_state"] = self._calc_kdj_similarity(
                candidate_features["kdj_state"], case_features["kdj_state"]
            )
        else:
            scores["kdj_state"] = 0.5

        # 3. 量能模式相似度
        if candidate_features.get("volume_pattern") and case_features.get(
            "volume_pattern"
        ):
            scores["volume_pattern"] = self._calc_volume_similarity(
                candidate_features["volume_pattern"], case_features["volume_pattern"]
            )
        else:
        ...
        if candidate_features.get("price_shape") and case_features.get("price_shape"):
            scores["price_shape"] = self._calc_shape_similarity(
                candidate_features["price_shape"], case_features["price_shape"]
            )
        else:
            scores["price_shape"] = 0.5

        # 加权总分
        total_score = sum(scores[k] * self.weights.get(k, 0.25) for k in scores)
    ...
    def _calc_trend_similarity(self, cand: dict, case: dict) -> float:
        ...
        # 4. 价格相对于短期趋势的偏离（百分比）
        cand_price_bias = cand.get(
            "price_vs_short_pct", cand.get("price_vs_short", 0) * 100 - 100
        )
        case_price_bias = case.get(
            "price_vs_short_pct", case.get("price_vs_short", 0) * 100 - 100
        )
            ...
            if len(cand_curve) > 0 and len(case_curve) > 0:
                if HAS_FASTDTW:
                    try:
                        ...
                        curve_sim = (
                            max(0, 1 - distance / max_dist) if max_dist > 0 else 0
                        )
                    except Exception:
    ...
    def _simple_dtw(self, seq1: np.ndarray, seq2: np.ndarray) -> float:
        ...
        if n != m:
            target_len = max(n, m)
            if n < target_len:
                seq1 = np.interp(np.linspace(0, n - 1, target_len), np.arange(n), seq1)
            if m < target_len:
                seq2 = np.interp(np.linspace(0, m - 1, target_len), np.arange(m), seq2)
