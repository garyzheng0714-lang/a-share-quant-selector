...
4. 若生产 data volume 还没有完整且新鲜的快照：已有完整但过期的验证快照时，发布前容器会先尝试日更；日更失败或完全没有验证快照时，才用独立容器从可信外部源全量重建。发布会先用只具有 `CHOWN`/`DAC_OVERRIDE` 能力的一次性容器校正数据卷所有者，实际引导进程仍使用非 root 身份。全量重建会把当前验证快照的股票池元数据（仅 `stock_names.json` / `universe_manifest.json`）种入空 staging，供东财主表故障时走腾讯确认的 LKG；不复用 legacy CSV。引导容器超时后仍在服务器继续执行，再次触发发布会继续等待；
