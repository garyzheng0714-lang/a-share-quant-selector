...
- 云阶决策不采集个股消息；浏览器 GET 和 worker 都不会为云阶候选调用个股新闻接口。
...
flowchart LR
    ...
    H --> I["云阶候选"]
    I --> K["结构 70% + 板块 30%"]
    K --> L["不可变决策 / 证据 / AI 解释账本"]
    L --> J["只读 Web/API 和 React 页面"]
```

发布成功的快照由内容 hash 生成 `snapshot_id`。`CURRENT_SNAPSHOT` 只在整批验证通过后原子切换，因此页面不会读到“一半新、一半旧”的数据。

收盘闭环固定为五步：采集/复用正式行情、确认快照、回填历史结果、模拟盘估值、生成派生指标和收盘决策。云阶命中后，系统只按“结构 70% + 板块 30%”形成可追溯优先级；消息和 AI 均不参与排序，AI 只解释结构、K 线和板块证据。上游任一步失败，下游不继续。
...
- 云阶候选的结构分、板块分、优先级和 AI 解释写入 `views.db` 的不可变 AI 决策运行记录。
...
与快照绑定的决策、云阶候选、结构/板块证据和 AI 运行记录同样按只追加账本保存。此前已写入的历史事件证据不会破坏性删除，但新云阶决策不再读取、展示或引用它们。

状态页会把该策略显示为已配置，并实时显示正式快照数和 staging 数，不再将“未设置自动清理天数”误报为异常。以后若要改成按天清理，必须先明确回溯期、异地备份与恢复流程，再通过一次明确的变更发布；当前版本没有任何自动清理任务。

## 7. 如何查当前真实状态

- 首屏 `/stocks`：先给出当日云阶数量、市场温度、主线板块、70/30 排序依据、第一/第二推荐和行业排名。
