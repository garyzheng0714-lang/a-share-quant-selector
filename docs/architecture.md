...
- 16:00 执行收盘 DAG：行情快照 → freshness → 模拟盘 → 板块/温度计/云阶/因子缓存 → 决策 → 候选近期消息 → 证据优先级 → AI 解释留痕；
...
- 云阶情报不由 Web 临时生成。Worker 只为当日云阶候选抓取近 30 日个股资讯，合并已落账公告事件，记录来源、发布时间、情报截止点和内容哈希；优先级由固定公式生成，AI 只解释引用到的事件 ID。抓取失败会降低覆盖等级，但不会把云阶买点偷偷降级成“观察”。
