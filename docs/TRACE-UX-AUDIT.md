...
证据日期：2026-08-05。本文件保存当时工作树的验收证据，不代表当前分支或线上版本仍然通过。任何新发布都必须按 `/Users/simba/design.md` 重新完成 TRACE 验证并记录真实结果。当时的“真实 API 数据”只证明页面不是静态 mock；当前历史行情抓取器在双源失败后可能把随机 OHLCV 写入正式 CSV，因此这份记录也不能证明底层数据来源真实。
