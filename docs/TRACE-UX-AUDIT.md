...
> 第 9 节记录的是当时已经发布的历史方案。消息相关产品路径已由下面第 10 节取代；历史账本不做破坏性删除。

## 10. 云阶消息面移除 TRACE 验收（2026-08-09）

- 产品目的：让用户只依据云阶结构、板块共振和市场温度完成当天判断，移除过期或不准确消息造成的认知干扰。
- 目标用户：每天直接执行云阶研究结论的单一决策用户。
- 核心任务：确认是否命中、第一/第二推荐、所属行业与热度、70/30 优先级和是否值得买入。
- 期望情绪：安静、清楚、可控，不需要先过滤消息噪音。
- 关键约束：不再抓取个股新闻；消息不进入排序、AI 输入、API 或页面；历史不可变账本继续保留；云阶仍是唯一入围规则。

| TRACE 项 | 真实证据 |
| --- | --- |
| T · 任务路径 | `/stocks` 首屏显示“2 只符合”→ 市场温度 44 → 主线有色金属 89 → 排序依据 70/30 → 第一/第二推荐；候选详情只保留 K 线、云阶三段、行业共振、下一步和结构失效风险。 |
| R · 可恢复交互 | 威派格与朗姿股份可切换；历史记录、同板块对比、图表设置和板块全景仍可展开/收起。 |
| A · 可访问状态 | 桌面和手机 DOM 均不存在“消息面 / 利好 / 利空 / 全部新闻”；候选、周期和详情按钮保留语义名称。 |
| C · 认知负担 | 原“情报覆盖”改为固定“70 / 30”排序口径；板块全景移除事件列表，只显示热度、趋势和广度。 |
| E · 证据闭环 | 2026-08-07 快照重算得到威派格 88.8（结构 98.6、板块 66.1）与朗姿股份 78.6（结构 90.0、板块 51.9）；新 AI 产物为 `cloud-stair-explainer-v3`、排序方法为 `cloud_stair_priority_v2`，候选中无消息字段。 |
| 桌面视觉 | 1600×1000：`qselect-cloud-no-news-desktop.png`；`clientWidth=scrollWidth=1593`，K 线实际绘制。 |
| 手机视觉 | 390×844：`qselect-cloud-no-news-mobile.png`；`clientWidth=scrollWidth=383`，K 线实际绘制。 |
| 运行证据 | 浏览器 warning/error 为 0；前端 lint/build 通过；后端 284 个测试和 9 个子测试通过；本次修改文件 Ruff 通过。 |

本地 TRACE 结论：适用硬门槛与红线均满足，可进入正式发布流程。全仓库 Ruff 仍会报告 `research/legacy/bowl_rebound_cli.py` 的 14 个既有遗留问题；该文件未被本次变更触碰，也不在当前发布检查路径内。
