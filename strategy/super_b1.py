...
def compute_super_b1(
    ...
    open_price = d["open"].astype(float)
    ...
    if uses_point_in_time_cap:
        ...
        if market_cap is None or market_cap <= 0:
            market_cap = (
                float(d["market_cap"].iloc[-1]) if "market_cap" in d.columns else 0.0
            )
    ...
    上涨十字星 = (C > REF(C, 1)) & (
        (C - open_price).abs() / open_price * 100 * 涨跌放宽系数 < 1.8
    )
    ...
    双叉戟 = (
        EVERY(长期 >= 75, 8) & (COUNT(短期 <= 50, 6) >= 2) & (COUNT(短期 <= 20, 7) >= 1)
    )
    红肥绿瘦 = (COUNT(C >= open_price, 15) > 7) | (COUNT(C > REF(C, 1), 11) > 5)
    ...
    for i, vday in enumerate(vdays):
        ...
        c_v, o_v = C.iloc[i_v], open_price.iloc[i_v]
        c_v_prev = C.iloc[i_v - 1] if i_v >= 1 else np.nan
        not_big_green = bool((not np.isnan(c_v_prev)) and c_v >= c_v_prev) or bool(
            c_v >= o_v
        )
    ...
    # {定义趋势股}
    做上涨趋势 = (趋势白线 >= 大哥黄线) & (
        (C >= 大哥黄线) | ((C > 大哥黄线 * 0.975) & (C > open_price))
    )
    强趋势股 = (
    ...
    强势回踩不破 = (
        ((L距离白线 < 1) | (L距离BBI < 0.5)) & (C > 趋势白线) & (距离白线 <= 3.5)
    )
    ...
    原始B1 = (
        ...
        & 适当缩量
        & (
            ((C - open_price).abs() * 100 / open_price < 1.5)
            | 超缩量
    ...
    超卖超缩量B = (
        ...
        & (((C < open_price) & (V < REF(V, 1)) & (C >= 大哥黄线)) | (C >= open_price))
        & ((当日涨跌幅 < (2 + Z1)) | 上涨十字星)
        & 超缩量
        & 异动任一
    )
    ...
    if return_history:
        ...
        for i in range(MIN_BARS - 1, len(d)):
            fired_at = [
                name
                for name, series in all_signals.items()
