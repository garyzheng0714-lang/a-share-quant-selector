...
class _FakeManager:
    ...
    def read_stock(self, code, nrows=None):
        ...
        return pd.DataFrame(
            {"date": ["2026-08-05", "2026-08-04"], "close": [10.0, 9.5]}
        )
...
class FactorComposeApiTest(unittest.TestCase):
    ...
    def test_and_returns_intersection_with_matched_and_streak(self):
        with (
            mock.patch(
                ...
                side_effect=lambda m, keys, date="": self._cached(keys),
            ),
    ...
    def test_missing_factor_snapshot_blocks_whole_result(self):
        with (
            mock.patch(
                "utils.factor_scan.read_cached_factor_hits",
                side_effect=lambda m, keys, date="": self._cached(["a"]),
            ),
    ...
    def test_bad_params_rejected(self):
        with mock.patch.dict("strategy.factors.FACTOR_REGISTRY", {"a": {}}):
            self.assertEqual(
                self.client.get("/api/factor-compose?keys=nope").status_code, 400
            )
            self.assertEqual(
                self.client.get("/api/factor-compose?keys=a&join=xor").status_code, 400
            )
            self.assertEqual(
                self.client.get("/api/factor-compose?keys=a&date=../x").status_code, 400
            )
