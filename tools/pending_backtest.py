...
方法与 tools/resonance_backtest.py 一致：T+1 开盘买入、持有 N 天收盘卖出，
样本内 / 样本外两段互不重叠的历史都要过，只在一段有效 = 过拟合噪音。

用法（生产服务器 2 核，全市场很慢，建议抽样）：
    python3 tools/pending_backtest.py --sample 2500 --out data/pending_backtest.json
"""
import argparse
import json
import random
    ...
    try:
        ...
        for i in range(180, n - 1):
            ...
            rec = {
                ...
                "broke_out": False, "wait_days": None,
            }
...
def _summarize(records, label):
    ...
    for w in HOLD_WINDOWS:
        ...
        res["breakout"][f"ret_{w}"] = _stats([r.get(f"breakout_ret_{w}") for r in broke])
    return res
...
def main():
    ...
    ap.add_argument("--out", default="data/pending_backtest.json")
    args = ap.parse_args()
    ...
    cm = CSVManager("data")
    with open("data/stock_names.json", encoding="utf-8") as f:
        names = json.load(f)
    ...
    print(f"[预备队回测] {len(codes)} 只股票 × 2 段窗口，workers={args.workers}", flush=True)
    ...
    for wkey, (lo, hi) in WINDOWS.items():
        ...
        with ProcessPoolExecutor(max_workers=args.workers) as ex:
            for res in ex.map(_one_stock, tasks, chunksize=8):
                ...
                if done % 250 == 0:
                    print(f"  [{wkey}] {done}/{len(codes)} 只，累计 {len(records)} 个预备信号",
                          flush=True)
        result["windows"][wkey] = _summarize(records, f"{lo} ~ {hi}")
        print(f"[{wkey}] 完成: {json.dumps(result['windows'][wkey], ensure_ascii=False)}",
              flush=True)
