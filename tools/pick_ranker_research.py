...
用法（生产容器）：python tools/pick_ranker_research.py
"""
import json
import sys
...
from strategy.factor_lib import MA, FactorContext  # noqa: E402
...
def build(records, cm, label):
    ...
    for code, rs in by_code.items():
        ...
        for r in rs:
            ...
            if not f:
                continue
            f.update({"code": code, "date": r["date"], "ret": r[f"ret_{HOLD}"],
                      "n_other": len(r["factors"]) - 1, "cap": r.get("cap")})
            rows.append(f)
    d = pd.DataFrame(rows)
...
def evaluate(df, feat, label, reverse=False):
    ...
    return {
        "feat": feat, "dir": "小者优" if reverse else "大者优",
...
def main():
    cm = CSVManager("data")
    ins = json.load(open("data/resonance_backtest.json", encoding="utf-8"))["records"]
    oos = json.load(open("data/resonance_oos.json", encoding="utf-8"))["records"]
    ...
    FEATS = [("J", True), ("J", False), ("RSI", True), ("RSI", False),
             ("vol_ratio", False), ("vs_peak60", True), ("vs_peak60", False),
             ("vs_ma20", True), ("vs_ma20", False), ("vs_ma60", True),
             ("vs_ma60", False), ("pct", True), ("pct", False), ("n_other", False)]
    ...
    print("\n=== 两段都有效的排序特征 ===")
    if not winners:
        print("  无——云阶命中的票之间无法有效区分，只能按其他维度（如板块热度）做次要排序")
