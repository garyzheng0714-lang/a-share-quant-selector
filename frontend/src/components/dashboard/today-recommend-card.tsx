...
import { useQuantPick } from "@/lib/hooks";
import { useAppStore } from "@/lib/store";
import type { QuantPickStock, SignalStock } from "@/lib/api";
...
function RecommendRow({ item, list }: { item: QuantPickStock; list: QuantPickStock[] }) {
...
export function TodayRecommendCard() {
  ...
  const picks: QuantPickStock[] = data?.today_buy ?? [];
      ...
      <header className="flex items-center gap-2 px-4 pt-3.5 pb-2">
        ...
        {data?.core_factor && (
          <span className="ml-auto inline-flex items-center gap-1 rounded-full bg-surface px-1.5 py-0.5 text-[10px] text-ink-secondary">
            ...
            云阶 · T+5 双周期胜率 {data.core_factor.track?.oos?.win ?? "—"}%
          </span>
        )}
      </header>
